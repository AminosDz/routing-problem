...
class Solver():
    ...
    def solve(self, criteria = "max_cap", order = None, time_limit=1.9, nb_neighbors = 5):
        ...
        for demand in sorted_demands:
            if time.time() - tic < time_limit:
                flow = self.get_path_bfs(demand, criteria=criteria, nb_neighbors=nb_neighbors)
    ...
    def get_path_bfs(self, demand, criteria = "max_cap", nb_neighbors = 5):
        ...
        while queue:
            ...
            for i in list(self.instance.nodes_list[n].neighbors.keys())[0:nb_neighbors]:
...
flows = solver.solve(criteria="max_cap", order=True, time_limit= 1.6, nb_neighbors=5)
write_flows(flows)